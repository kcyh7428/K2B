#!/usr/bin/env python3
"""Generate the 4 image fixtures used by Commit 3 extract-attachment tests.
...
def make_test_128() -> None:
    ...
    img.save(HERE / "test-128.png", "PNG")
    print(f"wrote {HERE / 'test-128.png'}")
...
def make_dr_lo_card() -> None:
    ...
    img.save(HERE / "dr-lo-card.png", "PNG")
    print(f"wrote {HERE / 'dr-lo-card.png'}")


def make_invalid_gif() -> None:
    """Minimal 1x1 GIF. VLM should reject this (supports JPEG/PNG/WebP only)."""
    img = Image.new("RGB", (1, 1), "white")
    img.save(HERE / "invalid.gif", "GIF")
    print(f"wrote {HERE / 'invalid.gif'}")
...
def make_corrupted_png() -> None:
    ...
    (HERE / "corrupted.png").write_bytes(png_sig + ihdr_length + ihdr_type + ihdr_data + ihdr_crc)
    print(f"wrote {HERE / 'corrupted.png'} (truncated at IHDR on purpose)")
...
def main() -> None:
    ...
    make_invalid_gif()
    make_corrupted_png()
    print("\nAll 4 fixtures generated. See ../calibration-corpus.md for usage context.")
