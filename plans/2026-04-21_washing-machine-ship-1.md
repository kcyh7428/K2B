...
- `scripts/washing-machine/classify.sh` (NEW): calls `scripts/minimax-json-job.sh --prompt classifier-prompt.txt --input - --job-name washing-machine-classify`. **Future refactor opportunity** (deferred, not Ship 1 scope): when Track A's `scripts/mmx-classify.sh` ships per `self_improve_requests.md` R-2026-04-21-001 (queued post 2026-04-24 gate), Washing Machine's `classify.sh` becomes a thin delegator. Do NOT pre-build the generic helper here -- premature abstraction before a second consumer exists; `project_minimax-offload.md` Track A owns that extraction. Prompt v1.0 (see `prompts/washing-machine-classifier-v1.0.txt`, also new) enforces:
