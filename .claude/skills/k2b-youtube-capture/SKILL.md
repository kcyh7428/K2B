...
- Taste profile: `~/Projects/K2B-Vault/Notes/Context/youtube-taste-profile.md` (generated by observer)
...
3. Read `Notes/Context/youtube-taste-profile.md` (if it exists). Extract scoring adjustments and confidence level. If the file doesn't exist, use neutral defaults (all taste scores = 5).
...
5. For each query, run: `scripts/yt-search.sh "<query>" --max 10`
6. Deduplicate against youtube-processed.md and youtube-recommended.jsonl
7. Score each video (0-10) across five dimensions:
   ...
   d. **Duration fit** -- 15-45min for commute (0-10)
   e. **Taste fit** (0-10) -- base 5, then apply from taste profile:
      - Add channel boost/dampen if channel matches
      - Add topic boost/dampen if topics match
