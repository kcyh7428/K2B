...
3. Select the `up:` MOC and verify it immediately before writing: use Read or `mcp__obsidian__get_file_contents` on the candidate MOC note (`MOC_SJM-Work.md`, `MOC_TalentSignals.md`, or `MOC_Agency-at-Scale.md`). If the read returns file-not-found, empty content, or tool failure, use `up: "[[Home]]"` and mention the fallback in the response. Do not reuse an earlier MOC check after edits or tool calls; re-check at the final write point.
4. Create the note at `~/Projects/K2B-Vault/raw/meetings/YYYY-MM-DD_Meeting-Topic.md`
5. **Run the vault-writer post-write pass** (see Post-Write Contract below)
6. Update today's daily note to add `[[YYYY-MM-DD_Meeting-Topic]]` in the Meetings section
7. **Update related project notes**: If the meeting is tied to a project, use the vault-writer update workflow to:
...
8. If significant insights found, offer to create a standalone insight note in `wiki/insights/`
...
- Set `up:` to the appropriate MOC based on meeting domain (`[[MOC_SJM-Work]]` for SJM, `[[MOC_TalentSignals]]` for TalentSignals, `[[MOC_Agency-at-Scale]]` for Agency at Scale, `[[Home]]` for general meetings). Before writing, verify the chosen MOC file exists in the vault; if it does not, set `up: "[[Home]]"` and mention the fallback rather than creating a dangling MOC link.
