...
- **Kimi K2.7 Code** = worker: background analysis, compilation, contradiction detection, bulk extraction, and non-Codex review. Default text provider since 2026-04-25.
...
- Migration history: observer and all background scripts ran MiniMax M2.7 from 2026-04-08; text routing migrated to Kimi on 2026-04-25 and now uses Kimi K2.7 Code. MiniMax's subscription lapsed 2026-05-27, so `K2B_LLM_PROVIDER=minimax` is deprecated and disabled. No M2.5 callers remain.
...
**Two-hat dynamic.** A K2B Claude Code session wears the **K2B PM hat by default** — works on K2B itself (skills, vault, code, infra, scripts); session-ends through `/ship`. When Keith triggers `continue k2b investment` / `resume k2bi`, the same session switches to the **K2Bi PM hat** — advisory only on K2Bi (no direct K2Bi code edits, K2Bi vault docs and proposals ARE editable, paste-ins/handoffs drafted for K2Bi builder sessions to execute); session-ends by updating the K2Bi Resume Card per its own self-documented session-end procedure (replace-don't-append the single current checkpoint, archive the predecessor body to `K2Bi-Vault/archive/checkpoints/YYYY-MM-DD_<slug>.md` before replacing — full procedure owned by the Resume Card at `K2Bi-Vault/wiki/planning/index.md` per the **Project Resume Handles** doctrine; CLAUDE.md keeps the two constraint phrases visible here as a survivable summary so the discipline is recoverable if the Resume Card link ever breaks). Persist any drafted artifacts (paste-ins, plans, rulings) as standalone vault notes (typically in `K2Bi-Vault/proposals/`) so they survive across PM sessions without depending on transcripts. If a single session uses both hats, do both at end: `/ship` for the K2B work + Resume Card checkpoint replacement for the K2Bi PM work. The hat is set by the trigger phrase; the discipline is set by the hat.
...
- **OpenAI-built diff** (Codex, OpenAI Agents, OpenAI Responses): review with Kimi only: `scripts/review.sh ... --builder-family openai --primary kimi --no-fallback --wait`.
...
- **Other or mixed builder**: choose one independent reviewer and use `--builder-family other --primary <codex|kimi> --no-fallback`; record why that reviewer is independent.

Use reviewer key `kimi` for the live Kimi reviewer in `scripts/review.sh`. The historically named `scripts/minimax-review.sh` remains only a deprecated compatibility alias for direct callers; the review runner invokes `scripts/kimi-review.sh` and no longer falls back to the old shim. Do not recommend the `minimax` key for new review commands. Same-family fallback never counts as official independent review. `/ship` should refuse to proceed without Keith's explicit override if the builder-family-clean reviewer is unreachable and no other independent review checkpoint has run.
