...
**MVP test gate (mandatory -- added 2026-04-22, corrects the "shipped at 20%" pattern):**

Before ANY status transition to `shipped` (single-ship feature) OR before marking a ship row `state: gate-passed` / `in-measurement` (multi-ship feature), verify ALL of the following. Each failure blocks the transition on its own.

**Gate 1 (field exists):** The feature's `mvp:` frontmatter field exists and is non-empty.

**Gate 2 (binary named-bug shape):** The `mvp:` value describes a **binary named-bug test** -- a specific user-visible bug that must die, with clear pass/fail conditions. Vague values ("improve memory", "better retrieval", "ship the classifier") FAIL this gate. Deliverable-shaped values ("ship the Normalization Gate", "land the 5 shelves") FAIL this gate -- shipping a deliverable is not the same as killing a bug.
...
**Note on the commit-before-gate ordering:** `/ship` step 5 commits code BEFORE this gate runs at step 6. If the gate rejects, the commit is already on disk. That is by design -- the code commit is safe to keep; what the gate blocks is the `status: shipped` LABEL. The feature stays `in-progress` with the commit landed, and the next ship of the same feature must either supply the missing evidence or name a revised MVP. Keith can always force the transition manually after understanding the failure, but `/ship` itself must never paper over a gate failure.

Rationale: three of K2B's shipped memory features (`canonical-memory`, `session-end-capture`, `pipeline-hardening`) passed through `/ship` without a binary test gate and each shipped at ~20%; the underlying doctor-phone bug stayed alive for five days and was only caught when Keith hit it live. WMM adopted the binary MVP test explicitly and is the first memory feature actually shipping. This gate generalizes that discipline to every feature.
