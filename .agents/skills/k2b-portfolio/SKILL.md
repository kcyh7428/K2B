...
Runs `~/Projects/K2B/.agents/skills/k2b-portfolio/scripts/portfolio.sh` and shows the output verbatim. The script reads only canonical K2Bi vault paths -- no new data stores invented here.
...
| Active orchestrator flights | `K2B_VAULT_PATH/System/orchestrator/orchestrator.sqlite` (override path with `K2B_ORCH_DB`) | `tasks` rows with `status NOT IN ('done','failed','cancelled')` -- i.e. `ready`, `running`, `blocked`, `zombie`. Strictly read-only via a 3-rung read (see implementation notes): `mode=ro` (retried), then a `-wal`-gated `immutable=1` fallback for an at-rest WAL DB, with a post-read re-check that re-reads via `mode=ro` if a writer appeared mid-read. Missing DB = `(none)`; only after `mode=ro` retries fail AND (a `-wal` is present OR the immutable fallback also fails) = `⚠ orchestrator board unreachable`. | label (entity_key ticker, else command_key) · status · next-action (blocked → ⚠ unblock: reason; zombie → ⚠ needs reclaim; running → auto · running; ready+blocked_by → auto · waiting on `<task>`; ready → auto · queued) · age (heartbeat age for running/zombie, else created age -- timezone-offset-aware via Python `fromisoformat`, since the orchestrator stores UTC `+00:00` timestamps) |
...
```bash
bash ~/Projects/K2B/.agents/skills/k2b-portfolio/scripts/portfolio.sh
```

With a section filter:
```bash
bash ~/Projects/K2B/.agents/skills/k2b-portfolio/scripts/portfolio.sh strategies
```
...
- **Orchestrator SQLite read: `mode=ro` with bounded retry, then a `-wal`-gated `immutable=1` fallback.** The Active flights section reads `orchestrator.sqlite` (WAL-mode) via `file:...?mode=ro` -- NOT the `-readonly` flag (fails CANTOPEN on a WAL DB needing to create the `-shm`). `mode=ro` takes a shared lock, never checkpoints, and cannot mutate state. The read strategy has three rungs, strictly read-only throughout:
  1. **`mode=ro`, retried up to 5x with a 0.2s backoff** (happy path returns on the first try, no delay). This works whenever a writer is active (the `-shm` already exists then) and rides out the brief transient just after a writer exits while its `-shm` is rebuilt.
  2. **If `mode=ro` still fails AND no `-wal` sidecar exists, fall back to `immutable=1`, fingerprinted.** A WAL DB at rest (last writer checkpointed and removed `-wal`/`-shm`) cannot be opened `mode=ro` at all -- a read-only open can't create the `-shm` a WAL DB needs, so it fails CANTOPEN(14) *permanently*, and retrying never clears it. This was the false "unreachable" on an idle board discovered live 2026-05-31. With no `-wal` there is no active writer, so `immutable=1` reads a consistent snapshot. Because `immutable=1` is unlocked, the read is bracketed by a high-resolution main-file fingerprint (nanosecond `st_mtime_ns`+`st_ctime_ns`+size+inode via `python3 os.stat`); if a `-wal`/`-shm` appears OR the fingerprint changes across the read OR the fingerprint is empty, the snapshot is treated as suspect and discarded in favour of an authoritative `mode=ro` reread -- which now succeeds because the writer's `-shm` exists. Nanosecond `ctime_ns` is what catches a transient writer that updates pages in place and checkpoints within a single whole second (same size, same inode), which a whole-second mtime would miss. The fingerprint is deliberately python3-only with NO whole-second `stat` fallback: a low-resolution fingerprint cannot prove same-second safety, so trusting it would reopen the race; python3 is already required by this section (`ts_age_human`), and if it ever fails the empty fingerprint forces the reread rather than trusting unlocked rows. If that reread cannot be obtained, the section reports unreachable rather than render suspect rows.
  3. **If `mode=ro` fails WHILE a `-wal` exists, report `⚠ orchestrator board unreachable`.** A writer is active but the `-shm` could not be built; we do NOT drop to `immutable=1` here, because that would ignore the live WAL and return a stale snapshot. The locked-read guarantee is preserved for exactly the case where it matters.
  The earlier 2026-05-30 retry-only design assumed the post-writer transient was the *only* `mode=ro` failure; the 2026-05-31 idle-board CANTOPEN proved an at-rest DB is a second, permanent failure that retry can't fix -- hence the `-wal`-gated immutable fallback.
