...
function usage(): void {
  console.log(`
Usage:
  schedule create "<prompt>" "<cron>" <chat_id>   Create a scheduled task
...
Examples:
  ...
  schedule create "Summarize open action items" "0 21 * * 1-5" 123456789
`)
}
  ...
  switch (command) {
    ...
    case 'list': {
      ...
      for (const t of tasks) {
        ...
        console.log(`  Next run: ${new Date(t.next_run).toLocaleString()}`)
