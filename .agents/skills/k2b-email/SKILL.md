...
3. **When you create a draft, your reply MUST include both the draft body and the draft ID.** The user cannot authorize content they did not read. Subject-only previews do not enable a send.
4. **Only send when the user's most recent message contains the exact draft ID from the preview.** Bare "send", "send it", "yes", "ok", "go", "proceed" never send. Only `gws gmail users drafts send` on that ID is authorized -- `+send`, `+reply`, `+reply-all`, `+forward` stay blocked.
...
- You previously (in an earlier turn) showed Keith the draft's body AND its ID.
- Keith's current message contains the exact draft ID from that preview.
...
Report back: `Sent. Message ID: <id>`. On failure, show the raw error. Do not retry automatically.
...
2. Reply to Keith with the draft body and the draft ID. Example:
   ...
   Body:
     Hi Bruno,
     ...full body here, verbatim...

   Draft ID: r-12345abcdef
   ```
3. End the turn. Do NOT send.
4. In Keith's next turn, if his message contains the exact draft ID, run "Send a Confirmed Draft". Otherwise ask him to re-send including the draft ID.
...
```bash
python3 "$HOME/Projects/K2B/scripts/k2b-shared-append.py" usage --skill k2b-email --summary "action: DESCRIPTION"
```
