...
# Plan: docs/superpowers/plans/2026-04-15-fix2-atomic-4-index.md (Fix #2 REVISED)
#
# Usage:
...
#      each into place.
#   4. Appends the compile log entry via scripts/wiki-log-append.sh (Fix #1).
#      If the log append fails, exits 2 -- the indices are already written,
#      so loud failure is preferred over silent.
#
# Lock pattern: mkdir-only at /tmp/k2b-compile-index.lock.d, same as
...
VAULT_ROOT = os.environ.get(
    "K2B_VAULT_ROOT",
    os.path.expanduser("~/Projects/K2B-Vault"),
)
...
LOCK_DIR = os.environ.get(
    "K2B_COMPILE_INDEX_LOCK",
    "/tmp/k2b-compile-index.lock.d",
)
...
# Master total line, e.g. "**Total wiki pages: 116** (99 prior + ...)"
...
def main():
    ...
    try:
        ...
        # Stage 3: append to wiki/log.md via Fix #1 helper. If it fails, the
        # indices are already on disk; exit 2 surfaces that loudly so Keith
        # knows to audit rather than silently losing the audit-trail entry.
