...

{
  ...
  grep -q 'dhcp.@dnsmasq\[0\].localuse' "$d/expensive-commands.log" || fail "router SSH trace should capture dnsmasq localuse guardrail"
  grep -q 'nslookup controlplane.tailscale.com' "$d/expensive-commands.log" || fail "router SSH trace should capture Tailscale control-plane DNS resolution"
