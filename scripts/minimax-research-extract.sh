...
# writing the final raw/research/ note. Kimi only does the extraction.
#
...
# Guard against empty/whitespace-only content -- no point calling the text worker for nothing.
...
if ! echo "$cleaned" | jq -e . >/dev/null 2>/tmp/k2b-minimax-json-err.$$; then
  ...
  echo '{"error": "Invalid JSON from '"${K2B_TEXT_WORKER_NAME}"'", "jq_error": '"$(echo "$jq_err" | jq -Rs .)"', "raw_response": '"$(echo "$cleaned" | jq -Rs .)"'}' >&2
  exit 1
fi
rm -f /tmp/k2b-minimax-json-err.$$
