...
{
  d="$TMPROOT/remote-host-ignored"
  fakebin="$d/bin"
  helper="$d/fast-whisper.sh"
  ssh_marker="$d/ssh-ran.txt"
  mkdir -p "$fakebin"
...
SH
  cat > "$helper" <<'SH'
#!/usr/bin/env bash
echo "local whisper remains the only fallback"
SH
  chmod +x "$fakebin/yt-dlp" "$fakebin/ssh" "$helper"
  ...
  if ! PATH="$fakebin:$PATH" \
    ...
    cat "$err" >&2
    fail "local whisper should remain available"
  fi
  [[ ! -e "$ssh_marker" ]] || fail "remote host input must never invoke ssh"
  grep -q "local whisper remains" "$out" || fail "expected local fallback"
  grep -q "METHOD: groq-whisper" "$err" || fail "expected local method"
  echo "  PASS: remote host configuration cannot trigger SSH"
}
