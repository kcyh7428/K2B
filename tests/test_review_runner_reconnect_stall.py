...
class ReconnectStallDetectorTests(unittest.TestCase):
    ...
    def test_reconnect_then_nonzero_exit_before_threshold_keeps_child_rc(self):
        rc, _elapsed = self._run(
            (
                'echo "[codex] Codex error: Reconnecting... 5/5"; '
                'sleep 0.1; '
            ...
            reconnect_stall_s=3, heartbeat_s=1, deadline_s=20,
        )
        log = _read_log(self.log_path)
        self.assertNotIn("RECONNECT_STALL_DETECTED", log)
    ...
    def test_non_codex_reviewer_never_arms_reconnect_stall_detector(self):
        cmd = [
            "bash",
            "-c",
            'echo "[codex] Codex error: Reconnecting... 5/5"; sleep 60',
        ]
        with patch.object(rr, "KILL_GRACE_S", 1):
            rc = rr.run_one_reviewer(
                "minimax", cmd, "test_job", self.log_path, self.state_path,
                self.state, deadline_s=3, heartbeat_s=1,
                reconnect_stall_s=1,
            )
        log = _read_log(self.log_path)
        self.assertNotIn("RECONNECT_STALL_DETECTED", log)
    ...
    def _assert_malformed_preserves_armed_state(self, malformed: str):
        rc, _elapsed = self._run(
            (
                'echo "[codex] Codex error: Reconnecting... 5/5"; '
                f'echo "[codex] Codex error: Reconnecting... {malformed}"; '
                'sleep 60'
            ),
    ...
    # Gate 5: malformed N/M shapes preserve an already-armed final
    # reconnect state. A corrupted stdout/stderr interleave must not
    # silently disarm the detector after a valid 5/5.
    def test_gate5_zero_cap_reconnect_preserves_armed_state(self):
        self._assert_malformed_preserves_armed_state("0/0")

    def test_blank_line_after_final_reconnect_does_not_disarm(self):
        rc, _elapsed = self._run(
            (
                'echo "[codex] Codex error: Reconnecting... 5/5"; '
                'echo ""; '
    ...
    def test_gate5_attempt_above_cap_reconnect_preserves_armed_state(self):
        self._assert_malformed_preserves_armed_state("7/5")
...
class CrossAttemptIsolationTests(unittest.TestCase):
    ...
    def test_quality_gate_uses_current_attempt_output_only(self):
        self.log_path.write_text("[previous attempt] APPROVE\n")
        rc = self._run(
            'echo "plain output without verdict marker"; exit 0',
            reconnect_stall_s=0, heartbeat_s=1, deadline_s=15,
        )
...
class ReconnectRegexParserTests(unittest.TestCase):
    ...
    def test_does_not_match_unrelated_lines(self):
        for line in (
            ...
            "[codex] Review text: Reconnecting... 5/5",
        ):
            self.assertIsNone(rr._RECONNECT_RE.search(line),
                              f"false-positive on: {line!r}")

    def test_does_not_match_codex_error_inside_assistant_message(self):
        for line in (
            '[codex] Assistant message captured: {"summary": "the bug was Codex error: Reconnecting... 5/5"}',
            '[codex] Assistant message captured: {"finding": "Codex error: Reconnecting... 5/5 then silence"}',
            '  "Codex error: Reconnecting... 5/5",',
            '- Finding: the line "Codex error: Reconnecting... 5/5" arms the regex',
            'Codex error: Reconnecting... 5/5 then more context',
            '[codex] Codex error: Reconnecting... 5/5 - retrying anyway',
        ):
            self.assertIsNone(rr._RECONNECT_RE.search(line),
                              f"false-positive on prose: {line!r}")
