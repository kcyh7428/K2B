...
Keith's 2026-09-09 decision: automatic discovery, interactive capture with Codex and Kimi. Only `discover` may run unattended; it updates machine-local queue/status without provider calls or vault writes. Extraction and home reconciliation happen in user-requested Codex sessions. Kimi Code membership does not authorize scripted batch extraction. Legacy `job-a`/`catch-up` provider paths must not be scheduled or used for bulk backlog processing. No paid API route is enabled.
...
- The only authorized unattended K2B job on SJM is local conversation discovery. It must not invoke Kimi, edit the Vault, or start other dormant routines.
