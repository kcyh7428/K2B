...
- **MiniMax API** (minimaxi.com) -- image generation, video, music, and VLM/OCR (model `MiniMax-VL-01` at `/v1/coding_plan/vlm`, used by washing-machine). **TTS retired 2026-05-14**, **STT retired 2026-05-14** (Token Plan allocates zero quota for both; calls return `status_code 2049 invalid api key` even though the key is valid for image/video/music/VLM). **For ALL speech generation, use `scripts/gptsapi-speech.sh`**, NEVER `mcp__minimax__text_to_audio` (the MCP tool is still in the agent's tool list but its backend is dead). **For ALL audio transcription, use `scripts/gptsapi-transcribe.sh`** with Groq Whisper as fallback. The legacy `scripts/minimax-speech.sh` was deleted 2026-05-16; `scripts/minimax-transcribe.sh` does not exist and MUST NOT be created — MiniMax has no STT endpoint, any such script would call a dead URL. Text completion endpoints are reachable too (M2.7 et al.) but K2B routes text to Kimi by default; flip back with `export K2B_LLM_PROVIDER=minimax`. API key in `MINIMAX_API_KEY` env var. Scripts in `scripts/minimax-*.sh`.
