...
The Mini holds the GPTsAPI key in `~/.zshrc`. Pattern:

```bash
ssh macmini 'bash -s' <<'REMOTE'
set -euo pipefail
KEY=$(grep -E 'GPTSAPI_KEY' "$HOME/.zshrc" | grep -oE 'GPTSAPI_KEY=.*' | head -1 \
  | sed -E 's/^GPTSAPI_KEY=//; s/^["'"'"']//; s/["'"'"'].*$//')
[ -n "$KEY" ] || { echo "GPTSAPI_KEY not found in ~/.zshrc" >&2; exit 1; }
export GPTSAPI_KEY="$KEY"
...
./scripts/gptsapi-image.sh --prompt "$P1" --aspect-ratio 4:3 --slug my-panel1 --timeout 170
REMOTE
DATE=$(date +%F)
mkdir -p /tmp/deckbuild/<slug>   # create the local build dir BEFORE any scp/cp
scp -q "macmini:/Users/fastshower/Projects/K2B-Vault/Assets/images/${DATE}_image_my-panel1.png" /tmp/deckbuild/<slug>/
```
...
**Known intermittent failure:** `gptsapi-image.sh` has a malformed shell expansion in its error-handler (`${detail//<newline>...`) that aborts the script when the API returns an unexpected payload, sometimes when running 3 panels in parallel. **Workaround:** if a parallel batch returns a `bad substitution` error for any panel, retry that single panel on its own — it almost always succeeds. Do NOT add a `timeout` wrapper to compensate; macOS ships no `timeout` binary and that path was already rejected (session note 2026-06-15). Retry a failed panel at most twice; if it still fails, stop and report (the cause is usually a real API error masked by the bad-substitution bug, not something a third retry fixes).
...
```bash
echo -e "$(date +%Y-%m-%d)\tk2b-infographic\t$(echo $RANDOM | md5sum | head -c 8)\tbuilt infographic: <slug>" \
  >> ~/Projects/K2B-Vault/wiki/context/skill-usage-log.tsv
```
